...
keep <- c("SiteID", "sampleDate", "sampleTime", "Temperature", "file_name")
...
```{r}
nps.data <- nps.data %>% 
  rename(AgencyID = SiteID) %>% 
  mutate(SiteID = case_when(grepl("KATM_lbrooo", orig_SiteID) ~ "KATM_lbrooo",
                            grepl("LACL_lclaro", orig_SiteID) ~ "LACL_lclaro",
                            grepl("LACL_kijilo", orig_SiteID) ~ "LACL_kijilo",
                            grepl("KATM_naknlo", orig_SiteID) ~ "KATM_naknlo",
                            TRUE ~ orig_SiteID)) 
...
```{r}
nps.data %>% 
  filter(SiteID %in% c("KATM_naknlo", "KATM_lbrooo", "LACL_lclaro", "LACL_kijilo"), UseData == 1) %>% 
  mutate(dt = as.POSIXct(paste(sampleDate, sampleTime), format = "%Y-%m-%d %H:%M:%S")) %>% 
  ggplot(aes(x = dt, y = Temperature, color = orig_SiteID)) +
  geom_line() +
  facet_wrap(~SiteID)
