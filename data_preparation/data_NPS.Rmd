...
keep <- c("file_name", "sampleDate", "sampleTime", "Temperature")
...
# STOPPED HERE
bind 2021 to other data and complete flagging of sites with multiple loggers - level, sonde, and hobos.


# Data review

NOTES on data quality 5/21/21: the four sites with level loggers and temp loggers need further review. These are paired at the sites and there are a lot of bad data that needs to be flagged further. I emailed Krista some plots so she can tell me what loggers to move forward with. 
...
Compare data from level loggers to temp loggers at 4 sites.
...
lclaro site. Level logger data for 2009-2011, 2013-2018. Temp logger provides a good check, but only available for 2014-2018. Note that LACL_claro_continuous_wq (2010, 2012-2020) is also at this location and would probably be the best to use given the problems between the level and temp loggers - time offsets and temp differences.
...
* 2018 temp logger has two air spikes and only goes to mid-July. Temperatures are off by 1 degree again, level logger is colder. Drop data for this year?

Use sonde for this site -- pretty long time series. May have small gaps in summer for maintenance, fill in as needed. Level logger time stamp may drift. 

```{r}
lclaro <- nps.data %>% 
  filter(SiteID %in% c("LACL_lclaro_lvl", "LACL_lclaro_temp", "LACL_lclaro_continuous_wq"), UseData == 1) %>% 
  # filter(year(sampleDate) == 2017) %>%
  mutate(dt = as.POSIXct(paste(sampleDate, sampleTime), format = "%Y-%m-%d %H:%M:%S")) %>% 
  ggplot(aes(x = dt, y = Temperature, color = SiteID)) +
  geom_line(aes(linetype = SiteID))
...
lkijilo site, there are three loggers at same location with data from 2014-2020 except temp logger missing 2016. Note that stream water sites is on opposite bank so could have slightly different temperatures given width at sampling location (~30 m bankfull). Talk with Krista about which she prefers, but don't use level logger, either temp logger or stream water.
...
Stream water site directly downstream of a cold water creek on LB. Use temp logger, but fill in level logger for 2016.
...
Probably best to use level logger for this dataset. Temp logger has very short time series and the cont wq logger is logging some part of the stream that is colder... maybe deeper placement. But, in 2017, cont wq sensor is only one that doesn't get buried. Krista can look at level logger data for 2017 where the level logger switched and became colder than the sonde.
