...
For visualization and data QA/QC, we want SiteID associated with each dataset, which we need from the metadata. Read in metadata file and associate SiteID with AKOATS_ID.

```{r}
accs_md <- read_csv("data/ACCS-UAA/metadata/SiteLevelMetadata_Bogan.csv") %>%
  select(AKOATS_ID, Waterbody_name, SiteID, SourceName, Contact_person, Latitude, Longitude)

accs_md <- accs_md %>% 
  mutate(Waterbody_name = case_when(SiteID == "mussm15" ~ "Trib. to SF Koktuli",
                                    SiteID == "mutsk09" ~ "Trib. to SF Koktuli",
                                    SiteID == "mutsk02" ~ "Trib. to SF Koktuli",
                                    SiteID == "muekm23" ~ "Trib. to SF Koktuli",
                                    SiteID == "iltnr19" ~ "Trib. to Newhalen",
                                    TRUE ~ Waterbody_name))
...
Read in data from ACCS folder. Rename, transform, and eliminate columns to match final field names described in Project_notes.
Keep the UseData field created by NCEAS for KNB archive, they should have at least flagged the bad (highly negative) data at Bonanza.
...
accs.data <- accs.data %>%
  ...
  mutate(sampleDate = as.Date(mdy(sampleDate)),
...
accs.data <- left_join(accs.data, accs_md, by = "AKOATS_ID")
...
Code for getting waterbody names. Probably not needed....
...
Note that processes below were created to flag data after viewing daily values. We really need to check at the subdaily level.
Add DT field for dates and times so can look at hourly changes in flagging function.

```{r}
source("Temp_flags_function.R", local = knitr::knit_global())

accs.data.qa <- accs.data %>%
  mutate(dateTime = parse_date_time(paste(sampleDate, sampleTime), "Y-m-d H:M:S"))

accs.data.qa2 <- accs.data.qa %>% 
  group_by(SiteID) %>% 
  do(data.frame(add_temp_flags(.)))

accs.data.qa2 <- flag_low_temps2(accs.data.qa) 

accs.data.qa2 <- accs.data.qa2 %>% 
  mutate(flag_any = case_when((flag_highTemps == 1|flag_lowTemps == 1|flag_hourlyChange == 1|flag_dailyChange == 1) ~ 1,
                              TRUE ~ 0)) 

accs.data.qa2 %>% 
  count(flag_highTemps)

accs.data.qa2 %>% 
  count(SiteID, flag_lowTemps, UseData)

accs.data.qa2 %>% 
  filter(SiteID == "muekm23", flag_lowTemps == 1, UseData == 1)

plot_flags(accs.data.qa %>% filter(UseData == 1), "accs")
```
...
```{r}
accs.sites <- accs.data.qa %>% distinct(SiteID) %>% pull(SiteID)


pdf("output/accs data review.pdf")

for(i in accs.sites) {
  dat <- accs.data.qa %>% filter(SiteID == i)
  dat %>% 
    filter(UseData == 1) %>% 
    ggplot(aes(x = dateTime, y = Temperature, color = as.factor(flag_any))) +
    geom_line() +
...
p1 <- accs.data.qa %>%
  filter(SiteID == "Bonanza Creek", UseData == 1) %>% 
  ggplot(aes(x = dateTime, y = Temperature, color = as.factor(flag_any))) +
  geom_line() +
  facet_wrap(~year(sampleDate), scales = "free_x")
...
accs.data.qa %>% 
  filter(SiteID == "Bonanza Creek", UseData == 1, flag_any == 1) 
...
## Daily flags by Ben -- currently not run.
...
If further data is is identified to be flagged, edit the csv file at "data/flagged_data/ACCS_flagged_data.csv"
