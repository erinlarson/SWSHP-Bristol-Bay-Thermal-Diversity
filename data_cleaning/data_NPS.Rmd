...
```{r}
nps.metadata  <- read_excel("data/NPS Bartz/Site_Info.xlsx", skip = 4) 
...
```{r}
nps.data.2 <- read.csv("data/NPS Bartz/All_Pilot_Sites_20201104.csv") %>%
...
nps.data.2 <- nps.data.2 %>% 
  mutate(Agency_ID = case_when(site_name == "Little_Kijik_Water" ~ "LACL_lkijr_stream_water",
                                    site_name == "Kijik_Beach_Water" ~ "LACL_kijil_beach_water",
                                    site_name == "Chulitna_Water" ~ "LACL_chull_beach_water",
                                    site_name == "Tazimina_Water" ~ "LACL_tazir_stream_water",
                                    site_name == "Up_A_Tree_Water" ~ "KATM_upatc_stream_water",
                                    site_name == "Margot_Water" ~ "KATM_margc_stream_water",
                                    site_name == "Port_Alsworth_Water" ~ "LACL_porta_beach_water",
                                    site_name == "Tlikakila_Water_A" ~ "LACL_tlikr_stream_water",
                                    site_name == "Idavain_Water" ~ "KATM_idavc_stream_water",
                                    site_name == "Tlikakila_Water_B" ~ "LACL_tlikr_stream_water",
                                    site_name == "Savonoski_Water" ~ "KATM_savor_stream_water"))

nps.data.2 <- left_join(nps.data.2, nps.metadata %>% filter(!Waterbody_Name == "Tlikikila River B") %>% 
            select(Agency_ID, Waterbody_Name)) %>% 
  mutate(Waterbody_Name = case_when(site_name == "Tlikakila_Water_B" ~ "Tlikikila River B",
                                    TRUE ~ Waterbody_Name)) %>% 
  select(-site_name)
