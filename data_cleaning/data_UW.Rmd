...
uw.data <-list.files(path = paste0("data/Jackie Carter UW/Jackie Carter/"),
   ...
   map_df(function(x) read_csv(x,col_types = cols(.default = "c"),col_names = T) %>% 
    ...
    select(-X9, -MeasurementType, -YearSampled) %>%
...
```{r}
uw.data %>% 
  distinct(file_name, StationName) %>% 
  count(file_name) %>% 
  arrange(desc(n))
```

Look at date file formats. It looks like most are "%m/%d/%y" 2-digit year. But, Big Whitefish and Little Whitefish have different format, "%d-%b-%y". 
...
uw.data <- uw.data %>% 
  mutate(sampleDate = case_when(grepl("Whitefish", StationName) ~ as.Date(Date, format = "%d-%b-%y"),
...
Join and format all data frames that have been read-in
```{r}
...
uw_flags <- read.csv("data/flagged_data/UW_flagged_data.csv") %>%
