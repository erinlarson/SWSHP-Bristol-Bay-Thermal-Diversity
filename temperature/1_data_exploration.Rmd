...
b2 <- bb_temp %>% 
  mutate(mo_day = format(sampleDate, "%m-%d"),
         ...
         large_drainage=case_when(
           Name=="Mulchatna River"~"Nushagak",
           Name=="Egegik Bay"~"Egegik Bay",
           Name=="Lake Clark"~"Kvichak",
           Name=="Lake Iliamna"~"Kvichak",
           Name=="Lower Nushagak River"~"Nushagak",
           Name=="Naknek"~"Naknek",
           Name=="Nushagak Bay"~"Nushagak Bay",
           Name=="Togiak"~"Togiak",
           Name=="Upper Nushagak River"~"Nushagak",
  ...
  geom_line(data = . %>% filter(is_19 == 1), 
            aes(x = as.Date(mo_day, format = "%m-%d"), y = meanDT, group = site_year, color = "red")) +
  geom_abline(aes(intercept = 18, slope = 0), linetype = 2) +
  facet_wrap(~factor(large_drainage, level=c("Egegik Bay", "Naknek", "Kvichak", "Nushagak", "Wood River", "Nushagak Bay", "Togiak"))) +
  theme_minimal() +
  theme(legend.position = "none", axis.title.x = element_blank(), text = element_text(size = 16)) +
  labs(y = "Mean Daily Temperature (°C)")
...
bb_ltmets %>%
  left_join(md %>% select(Site, Waterbody_name, HUC8), by = c("SiteID" = "Site")) %>%
...
ggplot(data=BB_mets%>%filter(Year %in% 2015:2019), aes(x=as.Date(MA7d_DMT_jd, origin="1970-01-01")))+geom_histogram(aes(fill=Year), binwidth=1)+theme_bw()+xlab("Month")+ylab("MWMT Count by Day")

ggsave("output/SWIM_figures/MWMTByDay.jpg", width=5, height=3, units="in", dpi=1000)
...
ggplot(BB_largedrainage_metrics, aes(x=factor(large_drainage, level=c("Togiak", "Nushagak Bay", "Wood River","Nushagak", "Kvichak", "Naknek", "Egegik Bay")), y=values, color=large_drainage))+ scale_color_brewer(type="div", palette="Dark2")+xlab("Watershed")+geom_boxplot(lwd=0.75)+facet_wrap(~metricf, scales="free", strip.position="bottom", nrow=1)+theme_bw()+coord_flip()+ylab(NULL)+guides(color="none")

ggsave("output/SWIM_figures/Bristol Bay Large Drainage boxplots.jpg", width=15, height=5, units="in", dpi=1000)
