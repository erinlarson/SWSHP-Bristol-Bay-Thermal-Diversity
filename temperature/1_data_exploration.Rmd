...
ggplot(all_mets_long, aes(x=fct_rev(Region), y=values))+xlab("Region")+geom_boxplot()+facet_wrap(~metricf, scales="free", strip.position="bottom")+theme_bw()+coord_flip()+ylab(NULL)
ggsave("output/SWIM_figures/all region boxplots.pdf", width=15, height=5, units="in", dpi=1000)
...
ggplot(BB_huc_metrics, aes(x=Name, y=values))+xlab("Watershed")+geom_boxplot()+facet_wrap(~metricf, scales="free", strip.position="bottom")+theme_bw()+coord_flip()+ylab(NULL)

ggsave("output/SWIM_figures/Bristol Bay HUC8 boxplots.pdf", width=15, height=5, units="in", dpi=1000)
...
##Stopped here - site IDs not matched up correctly
mets11_HUC<-mets11%>%
  left_join(BB_huc_metrics)%>%
  filter(Region=="Bristol Bay")

mets_pca <- prcomp(mets11_HUC %>% select(MWMT:`Tim. MWMT`) , scale = TRUE) 
biplot(mets_pca)
#ggplot version of pca biplot (so can add black circles around sites in BB)
...
# first plot is just points
p1 <- mets_pca %>%
  augment(mets11_HUC %>% filter(Region == "Bristol Bay")) %>% # add original dataset back in
  ggplot()  +
  geom_point(aes(.fittedPC1, .fittedPC2, color = as.factor(Name)), size = 1.5) +
  geom_point(aes(.fittedPC1, .fittedPC2), size = 2, shape = 1, data = . %>% filter(Year == 2019)) + 
  # stat_ellipse(aes(.fittedPC1, .fittedPC2, color = as.factor(grp_6))) +
  theme_half_open(12) + 
  background_grid() +
