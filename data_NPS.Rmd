...
ggplotly(
  ...
  filter(Agency_ID == "KATM_savor_stream_water"
...
# streams & beaches
nps.names.1 <- data.frame(unique(nps.data$SiteID))
colnames(nps.names.1) <- "SiteID"

# streams & beaches
nps.names.2 <- data.frame(unique(nps.lakes$SiteID))
colnames(nps.names.2) <- "SiteID"

# join all nps sites
nps.names <- bind_rows(nps.names.1,nps.names.2) %>%
  mutate(test = "test")
# agh?

# copied above list into excel...

nps.sites <- read_excel("data/NPS Bartz/NPS_sitename_conflicts_resolved.xlsx", sheet = "Sheet3")

z <- left_join(nps.data,nps.names,by="SiteID")
...
x <- nps.lakes %>% 
  mutate(AKOATS_ID = NA,
         SiteID = paste0(Agency_ID,"_",Waterbody_Name),
         useData = 1) %>%
  select(AKOATS_ID,SiteID,Depth,sampleDate,sampleTime,Temperature,useData) 
