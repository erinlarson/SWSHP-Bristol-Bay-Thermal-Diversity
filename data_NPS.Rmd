...
# manually create join table to associate names in "Site_Info.xlsx" with names in "All_Pilot_Sites_20201104.csv"

# FYI: the two sites are 36m apart; but should still be QA/QC'd separately, so keep separate at this step
...
# Notes
# - There is not a consistent number of depth levels (ranges ~ 5 - 10 depths)

# working here to maybe adapt example from http://datacornering.com/how-to-combine-files-with-r-and-add-filename-column/

all_paths <-
  list.files(path = "data/NPS Bartz/Multilevel_Lake_Sites",
             pattern = "*.csv",
             full.names = TRUE)
...
# read in one file and make it a function?
file <- read.csv("data/NPS Bartz/Multilevel_Lake_Sites/KIJIL_01_temp_20201104.csv", skip = 3) %>%
  filter(X != "Timestamp (UTC-08:00)") %>%
  mutate(filename =gsub(".csv","",basename(file)))
...
nps.lakes <- list.files(path = "data/NPS Bartz/Multilevel_Lake_Sites",
         pattern="*.csv", 
         full.names = T) %>% 
   map_df(function(x) read_csv(x,skip = 15, col_types = cols(.default = "c"),col_names = F) %>% 
            mutate(filename=gsub(".csv","",basename(x))))
