...
nps.meta <- read_excel("data/NPS Bartz/Site_Info.xlsx", skip = 4) %>%
  ...
  select(Lat,Long,Agency_ID,Waterbody_Name) %>%
...
nps.meta.1 <- read_excel("data/NPS Bartz/NPS_sitename_conflicts_resolved.xlsx", sheet = "Sheet1") %>%
  select(AKOATS_ID,Waterbody_name,Agency_ID) %>%
  transform(AKOATS_ID = as.character(AKOATS_ID))

# AKOATS_ID disappaears at this step --- why?
z <- left_join(nps.meta,nps.meta.1,by = c("Agency_ID","Waterbody_name"))



z <- akoats.meta %>%
  mutate(SiteID = paste0(SiteID,"_",Waterbody_name)) %>%
  filter(Contact_person == "Krista Bartz")


z1 <- inner_join(akoats.meta,nps.meta, by = c("Latitude","Longitude","SiteID"))

x <- dat %>% filter(filename == "NPS_streams_beaches") 
x <- data.frame(unique(x$X2))




x# For NPS data, Agency_ID + Waterbody name is the unique ID.  Either of them alone do not provide a unique ID
```
...
# format usgs metadata to match other metadata
...
```{r}
z <- bind_rows(uw.meta,cik.meta,accs.meta,fws.meta)
