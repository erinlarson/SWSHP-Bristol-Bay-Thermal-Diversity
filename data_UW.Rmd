...
* examine Lynx Creek data t see if it consists of multiple sites, or just naming variations on the same site
...
Examine individual datasets with suspected issue one at a time by site/year with interactive plotly chart
```{r}
...
p <- uw.data %>%
  ...
  # modify SiteID one at a time here to visually inspect dataset
  filter(SiteID == "Aleknagik Hansen Creek",
         year == 2008) %>%
  ggplot(aes(day,daily_mean_Temperature, color = year)) +
  geom_point() +
  facet_wrap(. ~ SiteID) +
...
# create table of erroneous data to be flagged/excised
# @ data/flagged_data/UW_flagged_data.csv

# working on 11/10/20 : some exposure at Hanson crk 2008
