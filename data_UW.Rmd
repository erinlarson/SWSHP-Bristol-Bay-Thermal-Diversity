...
* Date and time formats for part of the "Aleknagik Wood River" file for 2009 have some errors and/or inconsistencies.  Some of the cells in the time column begin with ":". Some cells in the date column end with a single digit, and the year is sometimes formatted with 2 digits or 4 digits.  Some of these feastures are likely artifacts of some previous data sorting/cleaning process.
...
# specify datasets to be removed (one watershed, one year)
StationName <- "Aleknagik Wood River"
YearSampled <- "2009"
exclude_wr <- data_frame(StationName,YearSampled)
...
uw.data.1 <- uw.data.1 %>%
  transform(sampleTime = lubridate::hms(sampleTime),
            Temperature = as.numeric(Temperature)) 
...
* Among the whole data set, there are some max temps > 50 C, indicating some logger malfunction and/or air exposure.  There is also on minTemp of -21.5 C
